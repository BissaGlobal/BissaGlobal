  ...
  return (
    <main style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 20px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <nav style={{ marginBottom: 20, fontSize: 14 }}>
        ...
        <Link href="/rdc" style={{ color: '#0057B8', textDecoration: 'none' }}>RDC</Link>
      ...
      {hotels.length === 0 ? (
        <p style={{ color: '#777' }}>Aucun hébergement listé pour le moment. <Link href="/" style={{ color: '#0057B8' }}>Rechercher d’autres destinations →</Link></p>
      ) : (
      ...
      <section style={{ marginTop: 40 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
          {DRC_PROVINCES.filter((p) => p !== province).map((p) => (
            <Link key={p} href={'/rdc/' + slugify(p)} style={{ fontSize: 13, color: '#0057B8', textDecoration: 'none', border: '1px solid #e5e7eb', borderRadius: 999, padding: '4px 12px' }}>{p}</Link>
          ))}
        </div>
      </section>
